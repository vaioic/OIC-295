...
def correct_image_tiled(slide, reference_slide, shift, shift_level=2, output_level=0, tile_size=128):
    ...
    shift_factor = reference_slide.level_downsamples[shift_level]/reference_slide.level_downsamples[output_level]
    shift_corrected = (int(np.round(shift[0] * shift_factor)),
                       int(np.round(shift[1] * shift_factor)))
    ...
    for row in tqdm(range(num_rows)):
        for col in range(num_cols):
            ...
            image = slide.read_region(
                (x0 - shift_corrected[0], y0 - shift_corrected[1]),
                output_level,
                size=(tile_width, tile_height)
            )
